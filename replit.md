...
- **Frame.io V4 API Upload Issues (August 11, 2025)**: Discovered that Frame.io V4 API file upload endpoints are not working as documented. Multiple endpoint attempts failed with 404 "no route found" errors including `/files`, `/folders/{id}/files`, and `/accounts/{id}/folders/{id}/assets`. Implemented temporary mock file upload system to maintain application functionality while awaiting Frame.io V4 API endpoint corrections or updated documentation.
