...
- **Frame.io API Limitations Confirmed (August 9, 2025)**: Comprehensive 5-step diagnostic confirmed that Frame.io developer tokens on personal accounts have extremely limited permissions. Only `/me` endpoint accessible (200 status), but `/accounts` returns empty array and all project/asset endpoints are inaccessible. This confirms personal accounts cannot use Frame.io API for project management, folder creation, or review link generation. System needs upgrade to Frame.io Pro/Business account or implementation of virtual folder system as workaround.
