 ...
 * This will find all existing shares and set them to expire 30 days from now
 */
...
async function updateExistingShares() {
  ...
  try {
    ...
    const frameioService = new FrameioV4Service();
    await frameioService.initialize();

    if (!frameioService.hasAccessToken) {
      console.error('❌ No Frame.io access token available. Please authenticate first.');
      process.exit(1);
    }

    // Get account ID
    const accountId = await frameioService.getAccountId();
    ...
    for (const project of projectsWithShares) {
      ...
      if (!shareId) {
        ...
        console.log('   Skipping this share...\n');
        continue;
      }
      ...
      try {
        ...
        const response = await frameioService.makeRequest(
          'PATCH',
    ...
    console.log(`All existing shares have been set to expire on: ${expirationISO}`);
